...
const ShipperChatsScreen = ({ navigation }: any) => {
  ...
  // Filter customer chats by search query and isOnline status (Active / InActive)
  const filteredCustomers = chatCustomers.filter(item => {
    if (selectedFilter === 'Active' && !item?.isOnline) return false;
    if (selectedFilter === 'InActive' && item?.isOnline) return false;
    if (searchQuery.trim()) {
  ...
  const renderChatItem = useCallback(
    ...
    [filteredCustomers.length, handleOpenChat],
  );
