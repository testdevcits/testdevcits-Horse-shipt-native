...
const SignupFlowScreen = ({ navigation }: any) => {
  ...
  const getPasswordScore = () =>
    [hasLength, hasNumber, hasSymbol].filter(Boolean).length;
  const barWidth =
    getPasswordScore() === 0
      ? '0%'
      : getPasswordScore() === 1
  ...
  const handleInitialSignup = async () => {
    ...
    if (!emailRegex.test(email))
      return setErrors(p => ({ ...p, email: 'Invalid email' }));
        ...
        <View
          ...
          style={[
            styles.stepLine,
            {
              width: 40,
              backgroundColor:
                item <= current ? COLORS.primary : COLORS.grey200,
            },
          ]}
        />
      ))}
        ...
        <View style={styles.contentCard}>
          ...
          <RoleSelectionModal
            ...
            onSelectRole={async newRole => {
              ...
              showInfoToast(
                'Role Selected',
                `Switched signup mode to ${newRole.toUpperCase()}`,
              );
            }}
          />
        </View>
