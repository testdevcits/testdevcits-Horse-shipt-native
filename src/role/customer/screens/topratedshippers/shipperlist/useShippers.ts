...
export const useShippers = () => {
  ...
  const fetchShippers = useCallback(
    async (isRefresh = false) => {
      try {
        ...
        const topList =
          topRes?.data ||
          (topRes as any)?.shippers ||
          (topRes as any)?.topRatedShippers ||
          [];
  ...
  const shippersWithFavState = useMemo(() => {
    ...
    return shippers.map(s => {
      ...
      const locationRegion =
        s?.region || s?.location || s?.address || s?.city || 'Region N/A';
  ...
  const filteredData = useMemo(() => {
    return shippersWithFavState.filter(s => {
      ...
      const matchesSearch =
        !qStr || nameStr.includes(qStr) || regionStr.includes(qStr);

      // 2. Quick filter
      let matchesQuick = true;
      if (activeFilters.quick === 'Verified') {
        matchesQuick = Boolean(
          s?.isVerified || s?.verified || (s?.rating || 0) >= 4,
        );
      ...
      const minRating =
        activeFilters.rating === 'All'
          ? 0
          : parseFloat(activeFilters.rating.replace('+', ''));
      const matchesRating = (Number(s?.rating) || 0) >= minRating;

      // 4. Category matches (If field exists check match, otherwise pass through)
      const matchesTransport =
        activeFilters.transport === 'All' || !s?.transportType
          ? true
          : s?.transportType === activeFilters.transport;
      const matchesExp =
        activeFilters.experience === 'All' || !s?.experienceLevel
          ? true
          : s?.experienceLevel === activeFilters.experience;
      const matchesResponse =
        activeFilters.response === 'All' || !s?.responseTime
          ? true
          : s?.responseTime === activeFilters.response;
      const matchesPrice =
        activeFilters.price === 'Any Price' || !s?.priceTier
          ? true
          : s?.priceTier === activeFilters.price;
